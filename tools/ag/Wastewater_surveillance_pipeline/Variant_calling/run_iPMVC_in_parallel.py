...
def run_core_iPMVC(wp_path,current_sample, nb_t_profiling):
	#unzip fastq.gz	
	os.system("gunzip -kv {0}{1}_R1.fastq.gz".format(wp_path, current_sample))
	os.system("gunzip -kv {0}{1}_R2.fastq.gz".format(wp_path, current_sample))
	...
	os.system("fastp -i {0}{2}_R1.fastq -I {0}{2}_R2.fastq -o {0}{2}_trimmed_1.fastq -O {0}{2}_trimmed_2.fastq -l 70 -x --cut_tail --cut_tail_mean_quality 20 --detect_adapter_for_pe --thread {1}".format(wp_path, nb_t_profiling,current_sample))
	os.system("prinseq-lite.pl -fastq {0}{1}_trimmed_1.fastq -fastq2 {0}{1}_trimmed_2.fastq -lc_method entropy -lc_threshold 60 -out_good {0}{1}_trimmed_lcfiltered -out_bad {0}{1}_trimmed_bad -out_format 1".format(wp_path, current_sample))
	...
	os.system("varscan pileup2snp {0}{1}.pileup --min-coverage 50 --min-reads2 1 --min-var-freq 0.25 > {0}out/variants_{1}.tab".format(wp_path, current_sample))
