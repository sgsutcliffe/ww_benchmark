...
input_workspace <- "./working2/" #as.character(commandArgs(TRUE)[1]) #TO UPDATE
output_workspace <- "./working2/" #as.character(commandArgs(TRUE)[2]) #TO UPDATE
...
nb_cores <- 1 #as.numeric(commandArgs(TRUE)[3]) #TO UPDATE
min_cov <- 0 #as.numeric(commandArgs(TRUE)[4]) #TO UPDATE
min_vaf <- 0 #as.numeric(commandArgs(TRUE)[5]) #TO UPDATE
...
#Get list of samples
lst_samples <- unname(sapply(X = (sapply(X = read.csv2(file = paste0(input_workspace,"lst_samples.txt"),sep = "\t",header = FALSE,stringsAsFactors = FALSE)[1], FUN=function(x) gsub(pattern = input_workspace,replacement = "",x,fixed = TRUE))),FUN=function(x) gsub(pattern = "_preprocessed_sorted.bam",replacement = "",x,fixed = TRUE)))
...
genome_refseq <- seqinr::getSequence(object = toupper(read.fasta(paste0(input_workspace,fasta_refseq_filename),seqtype = "DNA",as.string = TRUE,forceDNAtolower = FALSE)),as.string = TRUE)[[1]]

#import and concatenate samples variants calling file
df_variants <- read.csv2(file = paste0(input_workspace,"variants_",lst_samples[1],".tab"),sep = "\t",header = TRUE,stringsAsFactors = FALSE)
if (nrow(df_variants)!=0){
  df_variants$Sample <- lst_samples[1]
}
if (nb_samples > 1){
  for (i in 2:nb_samples){
    df_to_add_to_variants_df <- read.csv2(file = paste0(input_workspace,"variants_",lst_samples[i],".tab"),sep = "\t",header = TRUE,stringsAsFactors = FALSE)
...
#make sure that the variant frequency filter is applied (freq variant >=min_vaf on at least one strand)
df_variants$VarFreq <- (as.numeric(gsub(pattern = "%",replacement = "",x = df_variants$VarFreq,fixed = TRUE))/100)
df_variants <- subset(df_variants, (Reads1+Reads2>=min_cov)&(VarFreq>=min_vaf))
#apply strand bias filter on variants (freq variant >=0.01 on each strand)
df_variants <- df_variants[(((df_variants$Reads2Plus/(df_variants$Reads1Plus+df_variants$Reads2Plus))>=0.01)&((df_variants$Reads2Minus/(df_variants$Reads1Minus+df_variants$Reads2Minus))>=0.01)),]
...
df_sample_stratifications_of_interest <- read.csv2(file = paste0(input_workspace,"Table_Sample_stratifications_of_interest.csv"),sep = ";",header = TRUE,stringsAsFactors = FALSE)
...
# #amplicon positions
# df_amplicon_positions <- read.csv2(file = paste0(input_workspace,"amplicon_positions.csv"),sep = ",",header = TRUE,stringsAsFactors = FALSE)
# 
...
#     df_plot_mean_cov_per_sample_by_amplicon_current_subset$breadth_of_cov[the_i] <- length(unique(subset(df_depth,(sample==df_plot_mean_cov_per_sample_by_amplicon_current_subset$Sample[the_i])&(position>=start_current_amplicon)&(position<=stop_current_amplicon)&(depth>=min_cov)&(!is.na(depth)))$position))/length_current_amplicon
...
#df_plot_time_series_S_mutation_of_interest_prevalence_per_location$nb_samples_in_which_SNV_is_detectable <- unname(vapply(X = 1:nrow(df_plot_time_series_S_mutation_of_interest_prevalence_per_location),FUN = function(i) length(unique(intersect(subset(df_depth,(position==get_position_from_mut_name(the_mut_name = df_plot_time_series_S_mutation_of_interest_prevalence_per_location$mutation_name[i]))&(depth>=min_cov))$sample,names(table(names((v_samples_date[v_samples_date==df_plot_time_series_S_mutation_of_interest_prevalence_per_location$Sampling_date[i]]))))))),FUN.VALUE = c(0)))
...
df_prevalence_mut_of_interest_in_NCBI_WORLDWIDE_lineages$lineage[substr(df_prevalence_mut_of_interest_in_NCBI_WORLDWIDE_lineages$lineage,1,5)=="A.2.5"] <- "A.2.5.X"
df_prevalence_mut_of_interest_in_NCBI_WORLDWIDE_lineages$lineage[substr(df_prevalence_mut_of_interest_in_NCBI_WORLDWIDE_lineages$lineage,1,7)=="B.1.160"] <- "B.1.160"
#Import prevalence data TO ADD
df_sig_muts_prevalence_data_to_add <- read.csv2(file = paste0(input_workspace,"df_signature_muts_prevalence_VOCs_VUIs.csv"),sep = ",",header = T,stringsAsFactors = FALSE)
...
df_sig_muts_prevalence_data_to_add$lineage <- ifelse(df_sig_muts_prevalence_data_to_add$lineage=="A.2.5",yes="A.2.5.X",no=df_sig_muts_prevalence_data_to_add$lineage)
...
df_prevalence_mut_of_interest_in_NCBI_WORLDWIDE_lineages$lineage <- ifelse(substr(df_prevalence_mut_of_interest_in_NCBI_WORLDWIDE_lineages$lineage,1,5)=="A.2.5",yes="A.2.5.X",no=df_prevalence_mut_of_interest_in_NCBI_WORLDWIDE_lineages$lineage)
...
df_detected_marker_mutations_in_ww_samples$PANGO_lineage <- ifelse(df_detected_marker_mutations_in_ww_samples$PANGO_lineage=="A.2.5",yes="A.2.5.X",no=df_detected_marker_mutations_in_ww_samples$PANGO_lineage)
...
get_lst_lineage_mutations_and_conf_score <- function(the_lineage, the_sample){
  ...
  the_coverage_adjusted_confidence_score <- sum(v_current_lineage_signature_muts%in%unique(subset(df_variants,(Sample==the_sample))$label_mut_in_marker_fmt))/sum(v_site_depth_genomic_positions_signature_muts>=min_cov)
...
df_metadata_LSPQ_samples <- read.csv2(file = paste0(input_workspace,"LATEST_REPORT.csv"),sep = ",",header = TRUE,stringsAsFactors = FALSE)
df_metadata_LSPQ_samples <- subset(df_metadata_LSPQ_samples,(run.type%in%c("production"))&(sample.type=="clinical"))
df_HospCenter <- read.csv2(file = paste0(input_workspace,"ListeCH_prefix.csv"),sep = ",",header = TRUE,stringsAsFactors = FALSE)
...
the_f_parallel_preselect_based_on_nb_sig_muts_and_X_is_prevalence <- function(i_cl){
  ...
  for (z in the_vec){
    ...
    eval(parse(text=paste0("df_grid_search_combinations <- ",paste0("expand.grid(",paste0(rep("c(0.01,0.5,0.9)", ncol(X)),collapse = ","),")"))))
